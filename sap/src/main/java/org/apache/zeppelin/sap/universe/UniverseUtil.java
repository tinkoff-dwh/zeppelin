...
public class UniverseUtil {
  ...
  public UniverseQuery convertQuery(String text, UniverseClient client, String token)
    ...
    for (int i = 0; i < array.length; i++) {
      ...
      if (!selectPart && !wherePart && pathClosed && singleQuoteClosed
          && buf.toString().toLowerCase().endsWith("create")) {
      ...
      if (createPart) {
        if (pathClosed && singleQuoteClosed && buf.toString().toLowerCase().endsWith("as")) {
          ...
          Matcher m = p.matcher(create.toString().trim());
          if (m.find()) {
            createTableName = m.group(1).trim();
          } else {
            throw new UniverseException("Incorrect syntax of \"create table\"");
          }
        } else {
          create.append(c);
        }
